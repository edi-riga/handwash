    ...
    for i in range(len(is_washing)):
        ...
        if new_saved != old_saved:
            if new_saved:
                ...
                # filter out some of movement 0 frames
                if old_code != 0 or random.random() < MOVEMENT_0_PROPORTION:
    ...
    for subdir, dirs, files in os.walk(os.path.join(input_folder, folder)):
        for videofile in files:
            if videofile.endswith(".mp4"):
                ...
                # ffmpeg -r 30 -i frame_%d_snippet_2_2020-07-08_09-32-34_camera105.jpg -filter:v scale=320x240 out.mp4
                snippets = set()
                ...
                with open(snippet_extraction_file, "a+") as f:
                    for snippet, code in snippets:
                        ...
                        command = "ffmpeg -r 30 -i {}/frame_%d_snippet_{}_{}.jpg -filter:v scale=320x240 {}/snippet_{}_{}.mp4 >/dev/null 2>&1".format(ipath, snippet, vf, opath, snippet, vf)
                        f.write(command + "\n")
