...
#
# NOTE: this script requires that the input data is in the original (JPG image) format.
# It cannot be run on the Zenodo data. In order to separate frames in the Zenodo data,
# run the script seprate-frames.py instead.
#
...
def main():
    ...
    for movement in range(N_CLASSES):
        mk(os.path.join(out_frame_data_dir, "test", str(movement)))
        mk(os.path.join(out_frame_data_dir, "trainval", str(movement)))
        mk(os.path.join(out_video_data_dir, "test", str(movement)))
        mk(os.path.join(out_video_data_dir, "trainval", str(movement)))
